...
制約事項として、`:`と`.`を含めるページ名は特殊な用途で使用するためページ名に含めることはできません。

`:`はシステムで使用します。
例えば編集する場合、`https://.../MainPage:edit`みたいなパラメータとして使用します。

`.`は添付ファイルとページ名かの判定で使用します。
例えば、`https://.../MainPage/image.png`の場合、`MainPage`に貼り付けられた`image.png`という意味になります。
（新形式のrefプラグインの添付ファイルの貼り付け方の仕様準拠）

なお、直リンクを避けるため、直接このアドレスを開いた場合、ただちにダウンロードされるようにはしない予定です。
...
プラグインでしたがMarkdown準拠となり本体の機能で実装されます。動画、オーディオも貼ることができる。
...
例えばplantumlというプラグインがあり、{{ }}でくくられた部分をplantumlでUML画像に変換するという機能があった場合以下のようになります。
...
ここの仕様はまだ決まっていません。もしかするとコードの機能と統合する可能性もあります。
...
現在のところサポートする予定はありません。かりにサポートする場合は、:で始まるアドレスになります。